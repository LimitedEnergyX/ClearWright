...
class Handler(BaseHTTPRequestHandler):
    ...
    def _safe_write(self, body):
        """Write the response body, swallowing client-disconnect errors so a
        client that closed the socket mid-response is not surfaced as a server
        fault. ONLY the socket write is guarded; send_response, headers, and
        body construction stay outside this helper so genuine server errors
        still propagate."""
        try:
