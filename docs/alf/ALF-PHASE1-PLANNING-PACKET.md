...
- root_cause_confidence: 0.0 to 1.0; confidence_basis is a first-class mandatory string field stating the basis for that value; confidence values below 0.5 render prominently in operator review (section 16).
...
Canonical serialization (applies to every hashed artifact - observation identity tuples and files, finding records, revision lines, index and ledger lines, tier decisions, model input vectors, delta records): UTF-8, JSON with sorted keys, compact separators (comma and colon, no spaces), ensure_ascii off, exactly one trailing newline per record line, no floats in hashed identity material (confidence-like values are stored as fixed two-decimal strings inside hashed structures and as numbers only in display surfaces), and byte spans referenced by exact offsets. Two implementations following this section must reproduce identical hashes.
...
Attribution ledger (idempotency authority for synthesis): alf/attributions/ledger.jsonl, append-only under the same lock. One line per attribution: {attribution_id = sha256-16 over canonical {observation_id, entry_id, attribution_type}, observation_id, entry_id, attribution_type (initial_evidence | recurrence | regression | waste | delta_report), at, run_id}. Every counter update, recurrence link, regression reopen, waste accumulation, and delta line item MUST be accompanied by its ledger line written in the same locked write section; an attribution whose id already exists is a verified no-op (nothing increments twice). Re-running synthesis over the same observations is therefore idempotent end to end. The ledger is the authoritative answer to what has been counted.

Threat model (explicit, honest boundary): ALF integrity protection is application-level and tamper-EVIDENT, not physically append-only and not externally anchored WORM storage. Detected by hashes: any mutation of an observation file, finding head, revision line, index line, or ledger line that diverges from its recorded hash or hash chain. Detected by chained anchors: wholesale replacement of an index, history, or ledger file - each Run Improvement Delta carries an anchors block {observations_index_head_sha256, ledger_head_sha256, findings_revision_heads_sha256} computed over the current file bytes, and each delta additionally records the previous delta's anchor-block sha256, forming a chain; replacing history without forging every subsequent delta (which run summaries reference) breaks the chain and is reported by the verifier. NOT defended: an attacker with full filesystem write access who rewrites all ALF records, all deltas, and all referencing run records consistently - that is outside the supported threat model and is stated as such.
...
- Prominence rules in review: root_cause_confidence below 0.5, low-confidence merge proposals, and any Tier 0 entry render at the top with explicit markers. Regressions render as their own group. Authority-seeded findings (authority_seeded true) are visibly marked until observed-occurrence evidence attaches.
...
Goal: every governed ClearWright run produces (a) the normal governed work result and (b) an ALF Improvement Delta. Phase 1 enforcement (the claim is exactly this mechanism, no more): the delta generator runs at run boundaries (invocation-driven, same trigger as observation capture) and writes alf/deltas/rid-<run-id>.json; issuance is IDEMPOTENT per run_id - an existing byte-identical delta is a verified no-op, and a divergent rewrite attempt for the same run_id is REFUSED and reported (delta content is deterministic from the ledger, so divergence signals a defect); and a MISSING-DELTA VERIFIER (part of alf-verify) lists terminal governed runs (from summaries) that lack a delta and reports each as a Tier 1 lifecycle_failure finding candidate, so gaps are detected and escalated rather than silently accepted. Wiring delta generation into the terminal-summary path itself is a Phase 2 candidate and is NOT claimed here.
...
- evidence_references (object form {ref, sha256, role}):
  - {message:msg-20260713T211909139280, 08c3f3b351d1ead5ecf14edf389749ee609ef4bfcfb8f27828719814794ae3f3, observed_occurrence}
  - {message:msg-20260713T175640232571, 7fcf9c190f7095123db13403bc9954e15b09c0b85cc996567d6c8b9d28408263, observed_occurrence}
  - {summary:msg-20260719T181730501217, d5bbdcf3f1293c5e10c27553ac484a2abc24a8200a7e461f6b53d5713779b7cb, observed_occurrence} (mis-declared false start; operator close required)
  - {summary:msg-20260719T183202666915, db096c0a81f898944129fe35a04a0bc04bb1d8e6f8c3141d8f60e91b706bfa35, observed_occurrence} (complete refused verification_incomplete; operator close under recorded exception)
- root_cause_confidence: 0.9; confidence_basis: mechanism absence is directly inspectable in the CLI surface
- authority_seeded: true (with observed-occurrence evidence already attached)
...
- evidence_references (object form {ref, sha256, role}): {message:msg-20260719T181306094084, de155644f62e46370e6fbcfc4dd539d5dfcd2c9be6d71b7186f00f062d1fdb63, defining_authority} (run-level exemplars accrue at capture time from invocation-log and council-round observations as observed_occurrence entries)
- root_cause_confidence: 0.7; confidence_basis: detector absence is structural; recurrence dynamics not yet measured
...
- evidence_references (object form {ref, sha256, role}): {message:msg-20260719T181306094084, de155644f62e46370e6fbcfc4dd539d5dfcd2c9be6d71b7186f00f062d1fdb63, defining_authority} (subsystem cli plus queue_store; exemplars accrue from cleanup-bearing runs at capture time as observed_occurrence entries)
- root_cause_confidence: 0.8; confidence_basis: preflight-tool absence is structural; risk surface partially measured
- authority_seeded: true (visibly marked until observed-occurrence evidence attaches)
- status: TRIAGED; promotion_state: none; operator_disposition: none
