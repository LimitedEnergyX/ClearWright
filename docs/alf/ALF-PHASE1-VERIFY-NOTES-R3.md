# Verification Council round 3 - ALF Phase 1 (HIGH findings corrected, code-inclusive)

Round 2 found 4 HIGH defects; all are now corrected on branch operator/alf-phase1-impl
and the full inlined source below reflects the fixes. Full test suite: 1155 OK
(includes 22 new adversarial tests). Verify the corrections are complete and correct,
and identify any NEW HIGH or CRITICAL defect explicitly. Do not manufacture findings
or default pessimistic; approve when the evidence supports it.

## Reconciliation of the round-2 HIGH findings

HIGH-1 path traversal / ALF-root containment: FIXED. New safe_id() (rejects empties,
separators, drive/UNC, absolute, colon, '..', control chars) and safe_rel() (validates
every relative component) plus _contained() (normalized-absolute descendant check) are
applied to observation_file, finding_head_path/history_path, delta_path/snapshot_path,
render_spec (entry_id + positive-int version), Operation.append_line/replace_file, AND
_apply_staged_write during journal replay. Tests: test_safe_id_rejects_dangerous_and_accepts_valid,
test_path_helpers_reject_traversal, test_operation_rejects_traversal_target_rel,
test_recovery_rejects_traversal_journal_target, test_render_spec_rejects_bad_entry_and_version.

HIGH-2 torn-tail recovery corruption: FIXED. _heal_torn_tail() detects a final line
lacking a terminating newline, archives the torn bytes verbatim to alf/quarantine/, and
truncates to the last complete record BEFORE any append (called in _append_line_fsync via
the journal append, in _apply_staged_write's append path, and at recover() start). recover()
then runs verify_chain on the journal and FAILS CLOSED (IntegrityHalt) on any surviving
interior chain break rather than replaying an unauthenticated prefix. Tests:
test_torn_journal_tail_healed_not_corrupted, test_torn_target_tail_healed_on_append.

HIGH-3 compound-op atomicity: FIXED. Operation.commit(build=callback) runs the build
callback UNDER the writer lock, so every existence/idempotence/sequence/collision check
and the staged-write registration occur inside the SAME lock acquisition as the commit.
capture, create_finding (sequence allocation moved inside), update_finding,
attribute_occurrence, record_regression, surface_for_review, and dispose all use this.
A module-level _COMMIT_LOCK (RLock) additionally serializes threads within one process so
the transaction is atomic across BOTH threads and processes. Tests:
test_concurrent_identical_capture_no_duplication, test_concurrent_finding_creation_unique_ids.

HIGH-4 delta reference verification: FIXED. _verify_snapshot_refs() runs at the start of
_derive on every generation/rerun and hash-verifies EVERY snapshot occurrence line_sha256,
observation index sha256 AND file bytes, and attribution line_sha256; a missing, altered,
or unverifiable reference raises IntegrityHalt (no silent ignore). Tests:
test_observation_byte_tamper_fails_closed, test_observation_deletion_fails_closed,
test_rerun_noop_when_untouched.

Non-blocking also fixed: verify_hashes now verifies finding-history chains + head-equals-
rebuild; missing-delta-file with a surviving snapshot fails closed (AlfError); operator-
message binding uses a whole-token match (no substring false positives); promotion-gate
confidence comparison is type-safe (_conf_at_least coerces / fails closed). Tests:
test_message_binding_requires_whole_token, test_conf_type_safe,
test_promotion_gate_malformed_conf_no_crash, test_missing_delta_file_fails_closed.

## Review questions
1. Are the four HIGH corrections complete and correct, with no bypass path left?
2. Does safe_id/safe_rel/_contained cover EVERY externally-influenced filesystem path,
   including recovery replay? Any remaining unvalidated identifier or target_rel?
3. Is torn-tail healing safe (never discards a valid record; fails closed on chain breaks)?
4. Are compound mutations truly atomic under the combined in-process + cross-process locks?
5. Does the delta now fail closed on every divergent reference class?
6. Any NEW HIGH or CRITICAL defect introduced by the corrections?

---
# INLINED COMMITTED SOURCE (post-fix)


## SOURCE: tools/clearwright_dispatch_preflight.py (commit 6f1043d9362304255b4f44a8bea828a205e1f52f sha256 454fc9649e58edf29df4603ce55995f227a4396c426f5ce2d5248a588d6ec8a0)
```python
#!/usr/bin/env python3
...
## SOURCE: tools/clearwright_alf.py (commit 6f1043d9362304255b4f44a8bea828a205e1f52f sha256 35a5e5a8ef79dc5c3b0cc8668ca23ef9137093ae862d3559db18b7c3a257e368)
```python
#!/usr/bin/env python3
...
## SOURCE: tools/clearwright_alf_synth.py (commit 6f1043d9362304255b4f44a8bea828a205e1f52f sha256 74b31aa3844ada5e2b03b5a10319ab1bbe8564cdb8aecf0af3ea5461695ec5b2)
```python
#!/usr/bin/env python3
...
## SOURCE: tools/clearwright_alf_delta.py (commit 6f1043d9362304255b4f44a8bea828a205e1f52f sha256 b43d2d44237f5acede75319d1b781e567f43c3a4e3037323896778a9e8fd880d)
```python
#!/usr/bin/env python3
...
## SOURCE: tools/clearwright_alf_review.py (commit 6f1043d9362304255b4f44a8bea828a205e1f52f sha256 943546f61eaab24717de8a79d9077f162ca73f4f48c5d49f48fc5b4791219d0c)
```python
#!/usr/bin/env python3
...
## SOURCE: tests/test_alf_adversarial.py (commit 6f1043d9362304255b4f44a8bea828a205e1f52f sha256 8e939aaed87213e01e09b30602c5602d9eaf7b65aaa04e1a0057a89bc75b1714)
```python
...
## SOURCE: tests/test_alf_hardening.py (commit 6f1043d9362304255b4f44a8bea828a205e1f52f sha256 8718ecdf073b5b11e2c6cc5ec944a44e1a2bba9856f366724ac18ec79430321d)
```python
