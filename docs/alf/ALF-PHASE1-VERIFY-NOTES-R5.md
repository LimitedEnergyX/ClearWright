# Verification Council round 5 - ALF Phase 1 (round-4 completions, code-inclusive)

Rounds 2-4 corrected path-containment and delta-reference gaps; round 4's two findings
are now closed. Current committed source and tests are inlined. Full suite: 1164 OK.
Verify the corrections are complete with no bypass path; identify any NEW HIGH or
CRITICAL defect explicitly. Do not manufacture findings or default pessimistic; approve
when the evidence supports it.

## Reconciliation of the round-4 findings (both closed)

Symlink/reparse containment (both reviewers, HIGH): _contained now resolves with
os.path.realpath (not lexical abspath), so a symlink/junction/reparse-point planted
inside alf/ that points outside is REJECTED for every ALF read/write/replace/append and
recovery path. _rmtree is symlink-safe (a link is unlinked, never traversed) and the
orphan-staging cleanup unlinks planted links and realpath-contains any directory before
recursive removal, so cleanup can never delete outside alf/. (Additionally, round-4 GPT
nonblocking: _replace_bytes_fsync now fsyncs the parent directory after os.replace so a
committed replaced target survives power loss.) Tests: test_contained_rejects_symlink_escape,
test_rmtree_does_not_follow_symlink, plus the existing traversal tests.

Finding-history delta re-authentication (both reviewers, HIGH): _verify_snapshot_refs now
also runs verify_chain over every referenced finding-history chain, and
_resolve_finding_revision RECOMPUTES revision_sha256 over the revision payload (minus its
own hash and the outer chain fields) before accepting it - so an altered finding record
that retained a stale revision_sha256 and recomputed the outer line chain is caught and
fails closed. Test: test_altered_finding_revision_retained_hash_fails_closed (plus the
duplicate-revision and occurrence/ledger/index re-auth tests).

## Review questions
1. Is containment now symlink/reparse-safe on ALL ALF filesystem paths, including recovery
   staging, target writes, observation reads, and orphan cleanup?
2. Is delta reference verification now complete: every occurrence, observation, attribution,
   AND finding-revision reference re-authenticated (chain + recomputed hash), with duplicates
   and altered-with-retained-hash failing closed?
...
## SOURCE: tools/clearwright_dispatch_preflight.py (commit 59077c374131c4663a0c61b732e4a31f12e25d45 sha256 454fc9649e58edf29df4603ce55995f227a4396c426f5ce2d5248a588d6ec8a0)
```python
#!/usr/bin/env python3
...
## SOURCE: tools/clearwright_alf.py (commit 59077c374131c4663a0c61b732e4a31f12e25d45 sha256 7c904b06f97c0752fe096ecc326958ab920769d08890f9aa613df777080d2cb9)
```python
#!/usr/bin/env python3
...
## SOURCE: tools/clearwright_alf_synth.py (commit 59077c374131c4663a0c61b732e4a31f12e25d45 sha256 74b31aa3844ada5e2b03b5a10319ab1bbe8564cdb8aecf0af3ea5461695ec5b2)
```python
#!/usr/bin/env python3
...
## SOURCE: tools/clearwright_alf_delta.py (commit 59077c374131c4663a0c61b732e4a31f12e25d45 sha256 0d3c130f7eae0409c30961f77a7dc18c39ac9c6f8194042a17974d72616845e5)
```python
#!/usr/bin/env python3
...
## SOURCE: tools/clearwright_alf_review.py (commit 59077c374131c4663a0c61b732e4a31f12e25d45 sha256 fcf7e41709960033ef84bf17bdf767c1c2007b3f6aaa7e8863c72708e0ceeebc)
```python
#!/usr/bin/env python3
...
## SOURCE: tests/test_alf_adversarial.py (commit 59077c374131c4663a0c61b732e4a31f12e25d45 sha256 08024fbbc8d5c16812de98bdb5187ec446791b754f8dd40311c5410fd63bfaf1)
```python
